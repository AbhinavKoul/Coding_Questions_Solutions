...
class AAA
{
   friend ostream &operator<<(ostream &, const AAA &); // OUTPUT STREAM OVERLOADED!!!
   ...
   public:
      ...
      ~AAA(){}; 
      AAA &operator=(const AAA &rhs);           //overloaded equal 
      int operator==(const AAA &rhs) const;       // overloaded logical equal to
      int operator<(const AAA &rhs) const;      // overloaded less than
};
